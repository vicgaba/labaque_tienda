            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('categories.update', $category) }}" method="POST">
                        ...
                        <div class="mb-4">
                            <x-input-label for="active" :value="__('Estado')" />
                            <select id="active" name="active" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                <option value=1 {{ old('active', $category->active) == 1 ? 'selected' : '' }}>{{ __('Activo') }}</option>
                                <option value=0 {{ old('active', $category->active) == 0 ? 'selected' : '' }}>{{ __('Inactivo') }}</option>
                            </select>
