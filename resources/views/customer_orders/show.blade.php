{{-- resources/views/customer_orders/show.blade.php --}}
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Detalles del Pedido #') }}{{ $customerOrder->id }}
        </h2>
    </x-slot>
                    ...
                    <div class="mb-6">
                        ...
                        <p><strong>Estado:</strong>
                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{
                                $customerOrder->status === 'completed' ? 'bg-green-100 text-green-800' :
                                ($customerOrder->status === 'cancelled' ? 'bg-red-100 text-red-800' :
                                ($customerOrder->status === 'processing' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-800'))
                            }}">
                                {{ ucfirst($customerOrder->status) }}
                            </span>
                        </p>
                        <p><strong>Fecha del Pedido:</strong> {{ $customerOrder->created_at->format('d/m/Y H:i') }}</p>
                        <p><strong>Dirección de Envío:</strong> {{ $customerOrder->shipping_address ?? 'No especificada' }}</p>
                    </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @forelse ($customerOrder->customerOrderItems as $item)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                ${{ number_format($item->price_at_order, 2) }}
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
